<script lang="ts">
	...
	let pets = $state<AnimalItem[] | null>(null);
...
<div class="flex flex-col min-h-screen">
	...
	<div class="flex h-4/5 md:h-2/3 flex-col md:flex-row items-center mt-32 pt-10 pb-10">
		{#if userProfile === null}
			<div class="w-2/3 flex items-center justify-center">
				<svg aria-hidden="true" class="w-1/4 h-1/4 animate-spin dark:text-accent fill-secondary" viewBox="0 0 100 101"
			...
			<div class="w-1/3 flex flex-col justify-center items-center">
				...
				<button class="bg-white text-[#E91414] border border-secondary rounded-full w-3/5 mb-2 opacity-50" disabled>
					Supprimer le compte
				</button>
			</div>
		{:else}
			<div class="w-full md:w-1/3 flex justify-center items-center">
				<img src={userProfile.profilePicture}
						 class="md:h-64 h-48 md:w-52 w-40 mt-6 rounded-3xl shadow-[-12px_-12px_#FFDB78] md:shadow-[-24px_-24px_#FFDB78] object-cover"
						 alt="Profil" />
			</div>
			<form method="POST" action="?/updateUser" use:enhance={() => {
				...
				<div class="w-full lg:w-3/5 flex flex-col justify-center">
					...
					<div class="flex w-full pb-1">
						...
						<p
							class="w-1/2 lg:w-2/3 ml-2">{Math.abs((new Date(Date.now() - (new Date(Number(userProfile.birthday.split('/')[2]), Number(userProfile.birthday.split('/')[1]) - 1, Number(userProfile.birthday.split('/')[0]))).getTime()).getUTCFullYear()) - 1970)}</p>
					</div>
					<div class="flex w-full pb-1">
						<p class="w-1/2 lg:w-1/3 text-right text-secondary font-semibold">Anniversaire :</p>
						<p
							class="w-1/2 lg:w-2/3 ml-2">{formatter.format(new Date(Number(userProfile.birthday.split('/')[2]), Number(userProfile.birthday.split('/')[1]) - 1, Number(userProfile.birthday.split('/')[0])))}</p>
					</div>
					<div class="flex w-full pb-1">
					...
					{:else}
						<div class="flex w-full pb-1">
							...
							<textarea name="bio"
												class="border-2 border-accent rounded-lg pt-0.5 pb-0.5 pl-1 ml-2">{userProfile.bio}</textarea>
						</div>
					{/if}
				</div>
				<div class="w-full lg:w-2/5 flex flex-col justify-center items-center mt-5 md:mt-0">
					{#if !editHumanMode}
						...
						<button class="bg-accent text-white rounded-full w-3/5 mb-2" onclick={openAnimalModal}>Ajouter un animal
						</button>
						<button class="bg-white text-[#E91414] border border-secondary rounded-full w-3/5 mb-2">Supprimer le
							compte
						</button>
					{:else}
						<button type="submit" class="bg-accent text-white rounded-full w-3/5 mb-2">Sauvegarder</button>
						<button class="bg-white text-[#E91414] border border-secondary rounded-full w-3/5 mb-2" onclick={cancel}>
							Annuler
						</button>
					{/if}
				</div>
	...
	{#if pets === null}
		<div class="flex items-center justify-center bg-secondary bg-opacity-10 md:h-1/2 pt-10 pb-10">
			<svg aria-hidden="true" class="w-1/6 h-1/6 animate-spin dark:text-accent fill-secondary" viewBox="0 0 100 101"
					 fill="none" xmlns="http://www.w3.org/2000/svg">
				<path
					d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
					fill="currentColor" />
	...
	{:else}
		...
		{#if pets.length === 0}
			<div class="flex items-center justify-center bg-secondary bg-opacity-10 md:h-1/2 pt-10 pb-10">
				<p class="text-xl">Aucun animal disponible.
					<br>
					{#if $page.url.searchParams.get("empty") === "true"}
							Merci d'en ajouter un.
					{/if}
				</p>
			</div>
		{:else}
				...
				<form method="POST" action="?/updatePet" use:enhance
					...
					<div class="flex flex-col lg:flex-row justify-center max-[450px]:w-4/5 w-2/3">
						<div class="w-full lg:w-1/2 flex flex-col justify-center mt-5 lg:mt-0">
							<div class="flex w-full pb-1">
								<label class="max-[600px]:w-1/3 w-2/5 lg:w-1/3 text-right text-secondary font-semibold" for="firstName">Prénom
									:</label>
								<input type="text" class="border-2 border-accent rounded-lg pt-0.2 pb-0.2 pl-1 ml-2" id="firstName"
											 name="firstName" value={pets ? pets[currentPetIndex]?.firstName: ''} required />
							</div>
							<div class="flex w-full pb-1">
								<label class="max-[600px]:w-1/3 w-2/5 lg:w-1/3 text-right text-secondary font-semibold" for="age_pet">Âge
									:</label>
								<input type="number" class="border-2 border-accent rounded-lg pt-0.2 pb-0.2 pl-1 ml-2 w-16" id="age_pet"
											 name="age_pet" value="2" required />
								<p class="ml-1">ans</p>
							</div>
							<div class="flex w-full pb-1">
								<label class="max-[600px]:w-1/3 w-2/5 lg:w-1/3 text-right text-secondary font-semibold" for="birthday">Anniversaire
									:</label>
								<input type="date" class="border-2 border-accent rounded-lg pt-0.2 pb-0.2 pl-1 ml-2" id="birthday_pet"
											 name={pets ? pets[currentPetIndex]?.birthday: ''} required />
							</div>
							<div class="flex w-full pb-1">
								<label class="max-[600px]:w-1/3 w-2/5 lg:w-1/3 text-right text-secondary font-semibold" for="species">Espèce
									:</label>
								<input type="text" class="border-2 border-accent rounded-lg pt-0.2 pb-0.2 pl-1 ml-2" id="species"
											 name="specie" value={pets ? pets[currentPetIndex]?.species: ''} required />
							</div>
							<div class="flex w-full pb-1">
								<label class="max-[600px]:w-1/3 w-2/5 lg:w-1/3 text-right text-secondary font-semibold" for="breed">Race
									:</label>
								<input type="text" class="border-2 border-accent rounded-lg pt-0.2 pb-0.2 pl-1 ml-2" id="breed"
											 name="breed" value={pets ? pets[currentPetIndex]?.breed: ''} required />
							</div>
							<div class="flex w-full pb-1">
								<label class="max-[600px]:w-1/3 w-2/5 lg:w-1/3 text-right text-secondary font-semibold" for="weight">Poids
									:</label>
								<input type="text" class="border-2 border-accent rounded-lg pt-0.2 pb-0.2 pl-1 ml-2" id="weight"
											 name="weight" value={pets ? pets[currentPetIndex]?.weight: ''} required />
							</div>
							<div class="flex w-full pb-1">
								<label class="max-[600px]:w-1/3 w-2/5 lg:w-1/3 text-right text-secondary font-semibold" for="gender">Sexe
									:</label>
						...
						<div class="w-full lg:w-1/2 flex flex-col justify-center pl-5 lg:pl-0 pr-5 lg:pr-0 mb-5 lg:mb-0">
							<div class="flex flex-col">
								...
								<textarea
									class="border-2 border-accent rounded-lg pt-0.5 pb-0.5 pl-1 pr-1 ml-2 text-justify overflow-x-hidden"
									rows="10" cols="35" id="bio"
									name="bio">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum neque odio, eleifend at vehicula a, efficitur ac arcu. Cras malesuada ornare metus id imperdiet. Interdum et malesuada fames ac ante ipsum primis in faucibus. Maecenas varius nunc vitae dui tristique ullamcorper.</textarea>
							</div>
							<div class="flex mt-5">
					...
					<div class="flex flex-col lg:flex-row justify-center max-[450px]:w-4/5 w-2/3">
						<div class="w-full lg:w-1/2 flex flex-col justify-center mt-5 lg:mt-0">
							...
							<div class="flex w-full pb-1">
								...
								<p
									class="w-1/2 ml-2">{Math.abs((new Date(Date.now() - (new Date(pets ? pets[currentPetIndex]?.birthday : '')).getTime()).getUTCFullYear()) - 1970)}
									ans</p>
							</div>
							<div class="flex w-full pb-1">
						...
						<div class="w-full lg:w-1/2 flex flex-col justify-center pl-5 lg:pl-0 pr-5 lg:pr-0 mb-5 lg:mb-0">
							...
							<div class="flex mt-5">
								...
								<button class="bg-white text-[#E91414] border border-secondary rounded-full w-1/2 ml-5">Désactiver
								</button>
							</div>
						</div>
					</div>
